...
# ---- 5. 确保 .env 中 PLATFORM 为 arm64 ----
if [ -f .env ]; then
    sed -i.bak 's/^PLATFORM=.*/PLATFORM=linux\/arm64/' .env && rm -f .env.bak
fi
...
echo ""
echo "  请在 Wren UI 中配置 PostgreSQL 数据源："
echo "    Host: postgres  Port: 5432"
echo "    User: wrenai    Password: wrenai123"
echo "    Database: sales"
echo ""
echo "  示例问题: 每月总营收是多少？"
echo "========================================="
